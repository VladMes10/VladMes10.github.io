...
function MyProjects() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const regs = allApplications
        .filter(app => app.userId === uid)
        .map(app => app.competitionTitle)
  ...
  const handleSubmit = async (e) => {
    ...
    // Формую об'єкт даних для відправки
    const newProject = {
    ...
    try {
      ...
      if (response.ok) {
        resetFormFields();
        setShowForm(false);
        fetchData(); // Оновлюю список проєктів на сторінці
        setModal({ show: true, title: "Успішно", message: "Ваш проєкт успішно надіслано через сервер!", type: 'info' });
      } else {
        throw new Error("Сервер повернув помилку при збереженні");
      }
    } catch (error) { 
      console.error("Помилка відправки:", error);
      setModal({ show: true, title: "Помилка", message: "Не вдалося зберегти проєкт через сервер", type: 'info' }); 
    }
  };
